...
Chaque option a son rôle. `--driver=docker` dit à minikube de fabriquer le nœud sous la forme d'un conteneur Docker ; il sait aussi créer des machines virtuelles (pilotes `kvm2`, `virtualbox`) ou utiliser Podman, mais le conteneur est le plus léger et le plus rapide. `--cpus=2` et `--memory=4g` fixent ce que le nœud a le droit de consommer : ce sont des plafonds, pas des réservations, et nous verrons plus bas que le cluster au repos en utilise bien moins. `--kubernetes-version=v1.37.0` fige la version de Kubernetes ; sans cette option, minikube prend sa version par défaut, qui changera avec les prochaines versions de minikube.
